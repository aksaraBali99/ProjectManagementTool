...
@section('title', 'Edit company — Solava')
...
<div class="mx-auto max-w-2xl">
    ...
    <h1 class="mt-2 text-[14px] font-medium text-[#1F2937]">Edit company</h1>
    ...
    <form method="POST" action="{{ route('organizations.update', $organization) }}" class="mt-6 space-y-4" id="edit-organization-form">
        @csrf
        @method('PUT')
    ...
    <div class="mt-10 rounded-lg border border-gray-200 bg-gray-50 p-4">
        <h2 class="text-[12px] font-medium text-gray-700">
        ...
        <form method="POST" action="{{ route('organizations.toggle-active', $organization) }}" class="mt-3">
            @csrf
            @method('PATCH')
            <button type="submit" class="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-50">
...
@include('users._unsaved-changes-guard', ['formId' => 'edit-organization-form'])
@endsection
