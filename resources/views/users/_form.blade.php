...
<div id="user-form-tabs" data-initial-tab="{{ $initialTab }}">
    ...
    <div data-tab-panel="1" class="mt-4 space-y-4" style="{{ $initialTab === 1 ? '' : 'display: none;' }}">
        ...
        <div>
            ...
            <input id="employee_id" name="employee_id" type="text" value="{{ old('employee_id', $isEdit ? $user->employee_id : '') }}" required
                class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-[12px] focus:border-[#1D9E75] focus:outline-none focus:ring-1 focus:ring-[#1D9E75]">
            @error('employee_id')
                <p class="field-error mt-1 text-[11px] text-red-600">{{ $message }}</p>
            @enderror
                    ...
                    @foreach ($organizations as $organization)
                        ...
                        <div class="flex items-center justify-between rounded-md border border-gray-200 px-3 py-2">
                            ...
                            <select name="roles[{{ $organization->id }}]" data-org-id="{{ $organization->id }}"
                                class="role-select rounded-md border border-gray-300 px-2 py-1 text-[12px] focus:border-[#1D9E75] focus:outline-none focus:ring-1 focus:ring-[#1D9E75]">
