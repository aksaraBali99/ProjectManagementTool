...
2. Add an **Authorized redirect URI** matching that environment's callback URL, e.g. `http://solava.test/auth/google/callback` locally, or `https://your-domain/auth/google/callback` in dev/prod.
...
Each environment needs its **own** Google Cloud Console client and its own registered redirect URI — a client configured for `solava.test` won't work against a production domain.
